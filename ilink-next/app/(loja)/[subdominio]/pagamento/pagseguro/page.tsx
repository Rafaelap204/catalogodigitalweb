...
import { criarTransacaoPagSeguro } from '@/lib/server/actions/pagamentos';
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const psResult = await criarTransacaoPagSeguro({
          pedido_id: pedidoId,
        ...
        if (psResult.success && psResult.payment_url) {
          setPaymentUrl(psResult.payment_url);
        } else {
          setError(psResult.error || 'Erro ao criar pagamento');
        }
      } catch (error) {
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="text-sm text-gray-500">
                <p>
                  Você será redirecionado para o site do PagSeguro para 
                  completar o pagamento com total segurança.
                </p>
              </div>
            </div>
...
export default function PagSeguroPage() {
  return (
    <Suspense fallback={<PagSeguroLoading />}>
      <PagSeguroContent />
    </Suspense>
  );
}
