...
import { buscarSubdominio, deletarSubdominio, getTipoSubdominioLabel } from '@/lib/server/actions/subdominios';

export default function DeletarSubdominioPage() {
  ...
  const [subdominio, setSubdominio] = useState<{ id: number; subdominio: string; tipo: number } | null>(null);
  
  useEffect(() => {
    carregarSubdominio();
  }, [id]);
  
  async function carregarSubdominio() {
    const data = await buscarSubdominio(id);
    if (data) {
      setSubdominio({ id: data.id, subdominio: data.subdominio, tipo: data.tipo });
    } else {
  ...
  async function handleDeletar() {
    ...
    const resultado = await deletarSubdominio(id);
    
    if (resultado.error) {
      setErro(resultado.error);
      setDeletando(false);
    } else {
      router.push('/administracao/subdominios');
    }
  }
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-sm p-8">
          <div className="text-center">
            ...
            <p className="text-lg font-semibold text-blue-600 mb-2">{subdominio?.subdominio}</p>
            <p className="text-sm text-gray-500 mb-6">
              Tipo: {subdominio?.tipo ? getTipoSubdominioLabel(subdominio.tipo) : '-'}
              <br />
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/administracao/subdominios" className="flex-1 px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 flex items-center justify-center gap-2">
