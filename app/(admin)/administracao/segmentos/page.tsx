...
import { listarSegmentos, ListarSegmentosResult } from '@/lib/server/actions/segmentos';

export default function SegmentosPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const [dados, setDados] = useState<ListarSegmentosResult>({
    segmentos: [],
    total: 0,
    paginas: 0,
    paginaAtual: 1,
  ...
  async function carregarSegmentos() {
    setLoading(true);
    const resultado = await listarSegmentos({
      pagina,
      limite: 20,
  ...
  function mudarPagina(novaPagina: number) {
    if (novaPagina >= 1 && novaPagina <= dados.paginas) {
      router.push(`/administracao/segmentos?pagina=${novaPagina}`);
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <span className="text-gray-700">Segmentos</span>
          </div>
        </div>
        
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <form onSubmit={aplicarFiltro} className="flex gap-2 w-full sm:w-auto">
              <input
                type="text"
            ...
            <Link href="/administracao/segmentos/adicionar" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {dados.segmentos.map((segmento) => (
                      <tr key={segmento.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">#{segmento.id}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{segmento.nome}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${segmento.status === 1 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            {segmento.status === 1 ? 'Ativo' : 'Inativo'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                          <div className="flex justify-end gap-2">
                            <Link href={`/administracao/segmentos/editar/${segmento.id}`} className="text-blue-600 hover:text-blue-900 p-2">
                              <i className="lni lni-pencil"></i>
                            </Link>
                            <Link href={`/administracao/segmentos/deletar/${segmento.id}`} className="text-red-600 hover:text-red-900 p-2">
